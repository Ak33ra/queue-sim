...
Supports pluggable scheduling policies (FCFS, SRPT), multi-server tandem and feedback networks with probabilistic routing, and statistically rigorous output analysis via independent replications with confidence intervals.
...
tests/              64 tests — analytical validation, Little's law, property-based (Hypothesis)
```
...
- **Analytical (M/M/1):** E[T] = 1/(mu - lambda), E[N] = rho/(1 - rho), checked within 5% relative tolerance across multiple load levels
- **Little's Law:** E[N] = lambda * E[T] verified for both FCFS and SRPT
- **Confidence intervals:** 95% CI from `replicate()` covers the true M/M/1 E[T] on both Python and C++ backends
...
csrc/
  include/queue_sim/      C++ headers (distributions, server, FCFS, SRPT, queue_system)
